...
src/tiernav_runtime/
  ...
  recorder.py       + PromptAuditRecorder 类（独立写 prompt_audit/*.jsonl）
  cli.py            (新) dump_context_tokens() 函数
  __init__.py       + dump_context_tokens 导出
  prompts/          (新目录)
    task_instruction.py   外置的策略文本段
    __init__.py
```

注：prompt 审计走独立 `PromptAuditRecorder`，不走 `events.py` 的 `EpisodeEvent` 系统。两个 JSONL 文件职责分离：`events.jsonl` 记运行时事件，`prompt_audit/*.jsonl` 记 prompt 全文。审计文件直接含 content 全文，不需要从 event_log 重建。
...
新增 `PromptAuditRecorder` 类（在 `recorder.py`），每轮 compile 后写一行到 `<output_dir>/prompt_audit/<episode_id>.jsonl`：
...
- runtime tests 回归通过（基线 330 passed）
